...
public partial class ExamOfficer : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetProToExcel(ddldepartment.SelectedValue, "qry_examofficer");
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("ExamOfficerTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=ExamOfficerTemplate.xlsx");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatas(currentPage, objAdm.GetExamOfficer , ddldepartment.SelectedValue);
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                GetStaff();
                objAdm.getPanelByRecID(PnlExamOfficer );
                View.Visible = false;
                formTemplate.Visible = true;
            }
        }
        ...
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    officer_signature .Value = URL3;
                }
                if (itrig == 0)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, officer_signature.Value, active.SelectedValue, "INSERT", objAdm.ExamOfficer) == true)
                    {
                        ...
                        View.Visible = true;
                        formTemplate.Visible = false;
                        objAdm.clearPanel(PnlExamOfficer );
                        itrig = 0;
                ...
                if (itrig == 1)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, officer_signature.Value, active.SelectedValue, "UPDATE", objAdm.ExamOfficer) == true)
                    {
                        DisplaySuccess(objAdm.ErrorMessage);
                        LoadGrid();
