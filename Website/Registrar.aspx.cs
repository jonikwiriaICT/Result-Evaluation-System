...
public partial class Registrar : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetProToExcel(ddldepartment.SelectedValue, "qry_registrar");
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("RegistrarTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=RegistrarTemplate.xlsx");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatas(currentPage, objAdm.GetRegistrar , ddldepartment.SelectedValue);
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                GetStaff();
                objAdm.getPanelByRecID(PnlRegistrar );
                View.Visible = false;
                formTemplate.Visible = true;
            }
        }
        ...
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    registrar_signature.Value = URL3;
                }
                if (itrig == 0)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, registrar_signature .Value, active.SelectedValue, "INSERT", objAdm.Registrar) == true)
                    {
                        ...
                        View.Visible = true;
                        formTemplate.Visible = false;
                        objAdm.clearPanel(PnlRegistrar );
                ...
                if (itrig == 1)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, registrar_signature.Value, active.SelectedValue, "UPDATE", objAdm.Registrar) == true)
                    {
                        DisplaySuccess(objAdm.ErrorMessage);
                        LoadGrid();
