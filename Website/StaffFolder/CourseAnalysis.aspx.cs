...
public partial class StaffFolder_CourseAnalysis : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Lecturer() == "1")
        {
            var products = objAdm.DownloadAnalysis(Request.QueryString["CourseID"].ToString(), Request.QueryString["SessionID"].ToString());
    ...
    public string Administrator()
    {
        try
        {
            string username = ((Doctors_DoctorMaster)this.Master).Administrator;
            return username;
        }
        catch (Exception ex)
        {
        ...
        try
        {
            if (Lecturer() == "1")
            {
                if (!this.IsPostBack)
                {
                        ...
                        LoadGrid();
                        objAdm.SerializeGenderType(Request.QueryString["CourseID"].ToString(), Request.QueryString["SessionID"].ToString());
                        chartGender = objAdm.chartGender;
                        objAdm.SerializeMaritalStatus(Request.QueryString["CourseID"].ToString(), Request.QueryString["SessionID"].ToString());
                        chartMaritalStatus  = objAdm.chartMaritalStatus ;
                        objAdm.SerializeResultPercentage(Request.QueryString["CourseID"].ToString(), Request.QueryString["SessionID"].ToString());
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.AuditTrail(currentPage, "GetStatistics", Request.QueryString["CourseID"].ToString(), Request.QueryString["SessionID"].ToString());
