...
public partial class Program : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.getProgramInfoToExcel(ddlDepartment .SelectedValue);
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("ProgramTemplate");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Program Title in row " + RowNo + "");
                return;
            ...
            for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            if (objAdm.functionProgram(rec_id.Value , ddlDepartment .SelectedValue , columns[1].ToUpper ().ToString ().Trim () , "INSERT") == true)
            {
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/ProgramTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatas(currentPage, "GetProgram", ddlDepartment.SelectedValue);
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                objAdm.getPanelByRecID(PnlProgram);
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (string.IsNullOrEmpty(program_title.Text) || string.IsNullOrWhiteSpace(program_title.Text))
                {
                    DisplayError("Please enter Department Code.");
                }
                else
                {
                    string programTile = program_title.Text.ToUpper().ToString().Trim();
                    if (itrig == 0)
                    {
                        if (objAdm.functionProgram(rec_id.Value, ddlDepartment.SelectedValue, program_title.Text .ToUpper().ToString().Trim(), "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionProgram(rec_id.Value, ddlDepartment.SelectedValue, program_title .Text.ToUpper().ToString().Trim(), "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
