...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void NavigateClicked(object sender, EventArgs e)
    {
        try
        {
            if(string.IsNullOrEmpty (MatricNumber.Value) || string.IsNullOrWhiteSpace(MatricNumber.Value))
            {
                DisplayMessage("Please enter your matric number.", MsgType.Error);
            }
            if (string.IsNullOrEmpty(studentPassword.Value) || string.IsNullOrWhiteSpace(studentPassword.Value))
            {
                DisplayMessage("Please enter your User password.", MsgType.Error);
            }
            else
            {
                if (objAdm.GetStudentDetails(MatricNumber .Value , studentPassword.Value ))
                {
                    ...
                    Session["username"] = objAdm.sUsername;
                        ...
                        {
                            Response.Redirect("student-Change-Password");
                        }
