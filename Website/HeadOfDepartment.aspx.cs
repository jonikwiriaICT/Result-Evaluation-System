...
public partial class HeadOfDepartment : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetProToExcel(ddldepartment.SelectedValue, "qry_hod");
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("HodTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=HodTemplate.xlsx");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatas(currentPage, objAdm.GetHod , ddldepartment.SelectedValue);
        ...
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    hod_signature .Value = URL3;
                }
                if (itrig == 0)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, hod_signature.Value, active.SelectedValue, "INSERT", objAdm.Hod) == true)
                    {
                ...
                if (itrig == 1)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, hod_signature.Value, active.SelectedValue, "UPDATE", objAdm.Hod) == true)
                    {
                        DisplaySuccess(objAdm.ErrorMessage);
                        LoadGrid();
