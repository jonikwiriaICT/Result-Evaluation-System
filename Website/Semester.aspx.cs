...
public partial class Semester : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.getSemesterToExcel();
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("SemesterTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=Semester.xlsx");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Semester name in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            if (objAdm.functionSemester("", columns[1].ToUpper ().ToString ().Trim (), "INSERT") == true)
            {
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/SemesterTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetSemester");
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            objAdm.getPanelByRecID(PnlSemester );
            View.Visible = false;
            formTemplate.Visible = true;
        }
        catch (Exception ex)
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (string.IsNullOrEmpty(semester_name.Text) || string.IsNullOrWhiteSpace(semester_name .Text))
                {
                    DisplayError("Please enter Semester Name.");
                }
                ...
                else
                {
                    ...
                    if (itrig == 0)
                    {
                        if (objAdm.functionSemester(rec_id.Value, semester_name.Text.ToUpper().ToString().Trim(), "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionSemester(rec_id.Value , semester_name.Text.ToUpper ().ToString ().Trim (), "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
