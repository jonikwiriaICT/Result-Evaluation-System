...
public partial class Student : System.Web.UI.Page
{
    ...
    private void GetProgram()
    {
        try
        {
            DataSet ds = objAdm.GetProgram();
            ddlprogram .DataSource = ds;
            ddlprogram.DataValueField = "Code";
            ddlprogram.DataTextField = "Desc";
            ddlprogram.DataBind();
        }
        catch (Exception ex)
        {
    ...
    protected void SelectProgramClicked(object sender, EventArgs e)
    {
        try
        {
            LoadGrid();
    ...
    protected void SelectLevelClicked(object sender, EventArgs e)
    {
        try
        {
           if(ddlprogram .SelectedValue == "0")
            {
                DisplayError("Please select a Program.");
            }
            else
            {
                LoadGrid();
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetStudentToExcel(ddlprogram .SelectedValue, ddlLevel .SelectedValue );
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("StudentTemplate");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Matric Number in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            string s = Regex.Replace(columns[1].ToUpper().ToString().Trim(), @"[^\w\d]", "/");
            string p = s.ToString();
          
            if (objAdm.functionStudent(rec_id.Value   , ddlprogram .SelectedValue , ddlLevel .SelectedValue,p.ToString (), columns[2].ToUpper ().ToString (), "","", columns[4].ToUpper ().ToString (), columns[5].ToUpper ().ToString (), columns[3].ToUpper ().ToString (), columns[6].ToUpper ().ToString (), "img/grad.jpg", "INSERT") == true)
            {
    ...
    protected void ExcelInsertClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Administrator() == "1")
            {
                if (ddlprogram .SelectedValue == "0" && ddlLevel .SelectedValue =="0")
                {
                    DisplayError("Please select a Program and Level. ");
                }
                else
                {
                    ImporttoDatatable();
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/StudentTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDataStudent(currentPage, "GetStudent", ddlprogram.SelectedValue, ddlLevel.SelectedValue );
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                objAdm.getPanelByRecID(PnlStudent );
                View.Visible = false;
                formTemplate.Visible = true;
            }
        }
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                ...
                {
                    string s = Regex.Replace(mt_id.Text .ToUpper().ToString().Trim(), @"[^\w\d]", "/");
                    string p = s.ToString();
                    if (itrig == 0)
                    {
                        if (objAdm.functionStudent(rec_id.Value  , ddlprogram .SelectedValue , ddlLevel .SelectedValue , p.ToString (), firstname.Text.ToUpper ().ToString().Trim(), middlename .Text .ToUpper().ToString().Trim(),lastname.Text.ToUpper().ToString().Trim(), email.Text.ToUpper().ToString().Trim(), telephone_no .Text.ToUpper().ToString().Trim(), gender.SelectedValue, marital_status .SelectedValue , "img/grad.jpg", "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
