...
public partial class Session : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.getSessionToExcel ();
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("SessionTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=Session.xlsx");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Session Year from in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Session Year To in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            if (objAdm.functionSession("", columns[1].ToUpper().ToString().Trim(), columns[2], "INSERT") == true)
            {
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/SessionTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetSession");
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            objAdm.getPanelByRecID(PnlSession );
            View.Visible = false;
            formTemplate.Visible = true;
        }
        catch (Exception ex)
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (string.IsNullOrEmpty(session_year_from.Text) || string.IsNullOrWhiteSpace(session_year_from.Text))
                {
                    DisplayError("Please enter Session Year From.");
                }
                if (string.IsNullOrEmpty(session_year_to.Text) || string.IsNullOrWhiteSpace(session_year_to.Text))
                {
                    DisplayError("Please enter Session Year To.");
                }
                ...
                else
                {
                    ...
                    if (itrig == 0)
                    {
                        if (objAdm.functionSession(rec_id.Value, session_year_from.Text, session_year_to.Text, "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionSession (rec_id.Value, session_year_from .Text , session_year_to .Text , "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
