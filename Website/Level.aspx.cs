...
public partial class Level : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.getLevelToExcel();
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("LevelTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=Level.xlsx");
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/LevelTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetLevel");
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            objAdm.getPanelByRecID(PnlLevel );
            View.Visible = false;
            formTemplate.Visible = true;
        }
        catch (Exception ex)
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (string.IsNullOrEmpty(level_name.Text) || string.IsNullOrWhiteSpace(level_name.Text))
                {
                ...
                else
                {
                    ...
                    if (itrig == 0)
                    {
                        if (objAdm.functionLevel(rec_id.Value, level_name.Text.ToUpper().ToString().Trim(), "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionLevel(rec_id.Value, level_name .Text.ToUpper().ToString().Trim(), "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
