...
public partial class Department : System.Web.UI.Page
{
    ...
    private void GetFaculty()
    {
        try
        {
            DataSet ds = objAdm.GetFaculty();
            ddlFaculty.DataSource = ds;
            ddlFaculty.DataValueField = "Code";
            ddlFaculty.DataTextField = "Desc";
            ddlFaculty.DataBind();
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.getDepartmentInfoToExcel(ddlFaculty.SelectedValue);
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("DepartmentTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=DepartmentTemplate.xlsx");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Department name in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Department code in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            string departmentCode = department_code.Text.ToUpper().ToString().Trim();
            for (int j = 0; j < columnCount; j++)
            {
                columns[j] = row[j].ToString();
            }
            if (objAdm.functionDepartment(rec_id.Value, ddlFaculty.SelectedValue, columns[1], columns[2].ToUpper().ToString ().Trim(), "", "INSERT")== true)
            {
        ...
        try
        {
            ...
            if (Administrator() == "1")
            {
               if(ddlFaculty .SelectedValue == "0")
                {
                    DisplayError("Please select a Faculty. ");
                }
                else
                {
                    ImporttoDatatable();
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/DepartmentTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatass(currentPage, "GetDepartment", ddlFaculty.SelectedValue);
        ...
        try
        {
           if(ddlFaculty.SelectedValue == "0")
            ...
            else
            {
                ...
                objAdm.getPanelByRecID(PnlDepartment);
                ckEditor.InnerText = department_description.Value;
                View.Visible = false;
                formTemplate.Visible = true;
            }
        }
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (string.IsNullOrEmpty(department_name.Text) || string.IsNullOrWhiteSpace(department_name.Text))
                {
                    DisplayError("Please enter Department Name.");
                }
                if (string.IsNullOrEmpty(department_code.Text) || string.IsNullOrWhiteSpace(department_code.Text))
                {
                    DisplayError("Please enter Department Code.");
                }
                else
                {
                    string FacultyCode = department_code.Text.ToUpper().ToString().Trim();
                    if (itrig == 0)
                    {
                        if (objAdm.functionDepartment(rec_id.Value, ddlFaculty .SelectedValue , department_name.Text , department_code.Text .ToUpper ().ToString ().Trim () , ckEditor.InnerText , "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                            ...
                            View.Visible = true;
                            formTemplate.Visible = false;
                            objAdm.clearPanel(PnlDepartment);
                            ckEditor.InnerText = "";
                        }
                        else
                        {
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionDepartment(rec_id.Value, ddlFaculty.SelectedValue, department_name.Text, department_code.Text.ToUpper().ToString().Trim(), ckEditor.InnerText, "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
