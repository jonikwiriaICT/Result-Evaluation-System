...
public partial class SmsSettings : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.getSmsToExcel();
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("SMSTemplate");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Sms name in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Authentication ID in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            if (objAdm.functionSms ("", columns[1], columns[2], "INSERT") == true)
            {
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/SMSTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetSms");
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (string.IsNullOrEmpty(sms_name .Text) || string.IsNullOrWhiteSpace(sms_name.Text))
                {
                    DisplayError("Please enter SMS Name.");
                }
                if (string.IsNullOrEmpty(auth_id .Text) || string.IsNullOrWhiteSpace(auth_id.Text))
                {
                    DisplayError("Please enter Authentication ID.");
                }
                else
                {
                    ...
                    if (itrig == 0)
                    {
                        if (objAdm.functionSms(rec_id.Value, sms_name.Text.ToUpper().ToString().Trim(), auth_id.Text, "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionSms (rec_id.Value, sms_name.Text.ToUpper().ToString ().Trim (), auth_id .Text , "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
