...
public partial class CourseType : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetCourseType();
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("CourseTypeTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=CourseType.xlsx");
...

    {
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            {
                int RowNo = i + 2;
                DisplayError("Please enter Level name in row " + RowNo + "");
                return;
        ...
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            ...
            if (objAdm.functionLevel("", columns[1].ToUpper().ToString().Trim(), "INSERT") == true)
            {
    ...
    protected void downloadExcelTemplate(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                Download_File("~/facultyTemplate/CourseTypeTemplate.xlsx");
            }
            else
            {
                Response.Redirect("error");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetCourseType");
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            objAdm.getPanelByRecID(PnlCourseType );
            View.Visible = false;
            formTemplate.Visible = true;
        }
        catch (Exception ex)
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (string.IsNullOrEmpty(type_ref.Text) || string.IsNullOrWhiteSpace(type_ref.Text))
                {
                    DisplayError("Please enter Level Name.");
                }
                ...
                else
                {
                    ...
                    if (itrig == 0)
                    {
                        if (objAdm.functionCourseType (rec_id.Value, type_ref.Text.ToUpper().ToString().Trim(), "INSERT") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
                            ...
                            View.Visible = true;
                            formTemplate.Visible = false;
                            objAdm.clearPanel(PnlCourseType);
                    ...
                    if (itrig == 1)
                    {
                        if (objAdm.functionCourseType(rec_id.Value, type_ref.Text.ToUpper().ToString().Trim(), "UPDATE") == true)
                        {
                            DisplaySuccess(objAdm.ErrorMessage);
                            LoadGrid();
