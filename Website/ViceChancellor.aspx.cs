...
public partial class ViceChancellor : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetProToExcel(ddldepartment .SelectedValue, "qry_vice");
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("ViceTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=ViceTemplate.xlsx");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatas(currentPage, objAdm .GetVice , ddldepartment.SelectedValue);
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                GetStaff();
                objAdm.getPanelByRecID(PnlVice);
                View.Visible = false;
                formTemplate.Visible = true;
            }
        }
        ...
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    vice_signature.Value = URL3;
                }
                if (itrig == 0)
                    {
                        if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue , vice_signature .Value, active.SelectedValue ,"INSERT", objAdm.Vice) == true)
                        {
                    ...
                    if (itrig == 1)
                    {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, vice_signature.Value, active.SelectedValue, "UPDATE", objAdm.Vice) == true)
                    {
                        DisplaySuccess(objAdm.ErrorMessage);
                        LoadGrid();
