...
public partial class DeanOfFaculty : System.Web.UI.Page
{
    ...
    protected void ExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        if (Administrator() == "1")
        {
            var products = objAdm.GetProToExcel(ddldepartment.SelectedValue, "qry_dean");
            ExcelPackage excel = new ExcelPackage();
            var workSheet = excel.Workbook.Worksheets.Add("DeanTemplate");
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;  filename=DeanTemplate.xlsx");
    ...
    public void bindGrid(int currentPage)
    {
        ...
        TableResult.DataSource = objAdm.PopulateDatas(currentPage, objAdm.GetDean , ddldepartment.SelectedValue);
    ...
    protected void tableChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                GetStaff();
                objAdm.getPanelByRecID(PnlDean);
                View.Visible = false;
                formTemplate.Visible = true;
            }
        }
        ...
        try
        {
            if (Administrator() == "1")
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    dean_signature.Value = URL3;
                }
                if (itrig == 0)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, dean_signature .Value, active.SelectedValue, "INSERT", objAdm.Dean) == true)
                    {
                ...
                if (itrig == 1)
                {
                    if (objAdm.functionProcedure(rec_id.Value, staff_id.SelectedValue, dean_signature.Value, active.SelectedValue, "UPDATE", objAdm.Dean) == true)
                    {
                        DisplaySuccess(objAdm.ErrorMessage);
                        LoadGrid();
